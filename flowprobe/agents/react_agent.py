...
from flowprobe.agents.console import (
    console,
    elapsed,
    log_llm_decision,
    log_result,
...
from flowprobe.fingerprint import (
    ActionRecord,
    FingerprintRouter,
    FingerprintStore,
    SelectorRecord,
...
from flowprobe.graph import build_step_graph, cascade_step_failure, mark_steps_blocked, step_execution_order
from flowprobe.llm_client import LLMClient
from flowprobe.loader import ConditionStatus, Step, StepStatus, TestCondition
...
def run_test_condition(
    ...
    for step_id in order:
        ...
        produces_type = (step.produces or {}).get("type")
        ev = EvidenceCollector(run_id=run_id, claim_id=step_id, output_dir=output_dir, produces_type=produces_type)
...
def _trim_to_causal_chain(records: list) -> list:
    ...
    """
    from flowprobe.fingerprint import ActionRecord
...
def _react_loop(
    ...
    for step_num in range(1, max_actions + 1):
        ...
        for tool_call in msg.tool_calls:
            ...
            # If a download succeeded and this step declares produces.type, record
